...
# Presentation Plots ------------------------------------------------------
###########################################################################
#These are the same plots, but with no backgrounds and white text for the presentation
# Respondents by Sector ---------------------------------------------------
sectorpieplot <- ggplot(num_sector, aes(x = 1, y = prop, fill = Fleet)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  geom_text(aes(label = Freq, y=label_pos, x=1.3), color = "black", size = 8,fontface = "bold") +
  theme_void() +
  scale_fill_manual(values = c("#4472C4","#CCCCFF","#A6A6A6")) +
  #labs(title = "Responses by Sector") +
  theme(legend.position = "bottom",
...
print(sectorpieplot)
ggsave("Broad Sentiment Plots/No background/Responses by Sector Pie.png", 
       plot = sectorpieplot,
       width = 6.5, #Adjust width as needed
       height = 4.22, #Adjust height as needed
       units = "in", 
       dpi = 700)
...
overall_sent_plot <- ggplot(overall_sent, aes(x = 1, y = prop, fill = Sentiment)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  geom_text(aes(label = Freq),position = position_stack(vjust = 0.5), color = "black", size = 8, fontface = "bold") +
  theme_void() +
  scale_fill_manual(values = c("#92D050","#FFFF00","#ED7D31")) +
  labs(title = "Overall Comment Sentiment") +
  theme(legend.position = "bottom",legend.title = element_blank(),
...
ggsave("Broad Sentiment Plots/No background/Overall Sentiment.png", 
       plot = overall_sent_plot,
       width = 6.09, #Adjust width as needed
       height = 4.25, #Adjust height as needed
       units = "in", 
       dpi = 700)


# Overall Sentiment by Sector ---------------------------------------------
...
sect_sent_plot <- ggplot(sectorsent, aes(x = Fleet, y = Freq, fill = Sentiment)) +
  ...
  theme(legend.position = "bottom",
        ...
        axis.text = element_text(size = 12, color = "white", face = 'bold'),
        panel.grid.major.x = element_blank(),panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_line(color = 'white'),panel.grid.minor.y = element_blank())
print(sect_sent_plot)
ggsave("Broad Sentiment Plots/No background/Overall Sentiment by Sector.png", 
       plot = sect_sent_plot, 
       width = 6.7, #Adjust width as needed
       height = 3.87, #Adjust height as needed
       units = "in", 
       dpi = 700)

# Stock Condition Sentiment -----------------------------------------------

stock_sent_plot <- ggplot(stock_sent, aes(x = 1, y = prop, fill = Sentiment)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  geom_text(aes(label = Freq, y=label_pos, x=1.3), color = "black", fontface = 'bold', size = 8) +
  theme_void() +
  #scale_fill_manual(values = c("#ED7D31","#FFFF00","#92D050")) +
  scale_fill_manual(values = c("Negative" = "#ED7D31","Neutral/Mixed"  = "#FFFF00","Positive" = "#92D050"),breaks = c("Positive", "Neutral/Mixed", "Negative"))+
  labs(title = "Stock Condition Sentiment") +
  theme(legend.position = "bottom",
        legend.title = element_blank(),
        legend.text = element_text(size = 16, color = 'white', face = 'bold'), #Remove comment for pres.
        plot.title = element_text(hjust = 0.5, size = 16, color = 'white', face = 'bold')) #Remove comment for pres.
  

print(stock_sent_plot)

ggsave("Broad Sentiment Plots/No background/Stock Condition Sentiment.png", 
       plot = stock_sent_plot,
       height = 3.89, #Adjust width as needed
       width = 5.41, #Adjust height as needed
       units = "in", 
       dpi = 700)


# Stock Condition by Sector -----------------------------------------------
sect_stock_sent_plot <- ggplot(sectorstock_sent, aes(x = Fleet, y = Freq, fill = Sentiment)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6) +
  labs(title = "Stock Condition Sentiment by Sector", x = "", y = "", fill = "Subgroup") +
  scale_fill_manual(values = c("#92D050","#FFFF00","#ED7D31")) +
...
print(sect_stock_sent_plot)
#Save the plot to the filepath
ggsave("Broad Sentiment Plots/No background/Stock Condition Sentiment by Sector.png", 
       plot = sect_stock_sent_plot, 
       width = 6.52, #Adjust width as needed
       height = 4.13, #Adjust height as needed
       units = "in", 
       dpi = 700)
