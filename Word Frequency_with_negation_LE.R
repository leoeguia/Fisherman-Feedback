...
# Negation check ----------------------------------------------------------
negation_words <- c("no", "not", "never", "none", "not a")
out <- data.frame()
for(i in 1:length(rg)){
  tokens.i <- tibble(text = rg[i]) %>%
    unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
    tidyr::separate(bigram, into = c("word1", "word2"), sep = " ")
  x <- inner_join(tokens.i, new_sentiment, by = c("word2" = "word"))
  x$sentiment <- ifelse(x$word1 %in% negation_words,ifelse(x$sentiment == "positive", "negative", "positive"),x$sentiment)
  POS <- sum(x$sentiment == "positive")
  NEG <- sum(x$sentiment == "negative")
  SENT <- POS - NEG
  df <- data.frame(positive=POS, negative=NEG, sentiment=SENT)
  df$standardized <- ifelse(nrow(x) > 0, df$sentiment / nrow(x), NA)
  df$words <- nrow(x)
  out <- rbind(out, df)
}
out2 <- na.omit(out)
...
rg3$word_display <- ifelse(rg3$word1 %in% negation_words,
                           paste(rg3$word1, rg3$word2),
                           rg3$word2)

rg3$word_sentiment <- rg3$word2
bing_word_counts <- rg3 %>%
  inner_join(new_sentiment, by = c("word_sentiment" = "word")) %>%
  mutate(word=word_sentiment) %>% 
  mutate(sentiment = ifelse(word1 %in% negation_words,
...
par(mar = c(0, 0, 0, 0))
rg3 %>%
  inner_join(new_sentiment, by = c("word2" = "word")) %>%
  mutate(sentiment = ifelse(word1 %in% negation_words,ifelse(sentiment == "positive", "negative", "positive"),sentiment),
         word = ifelse(word1 %in% negation_words,paste(word1, word2),word2)) %>%
  count(word, sentiment, sort = TRUE) %>%
  acast(word ~ sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("#fc8d59","#99d594"),max.words = 30, title.size = 1.5,match.color = TRUE)
dev.off()
# Abundance People Only ---------------------------------------------------
### NEEDS NEGATION UPDATE
df<- read_xls("Gag_Fisherman Feedback_2026_CLEAN for analysis.xls") #Change this for different species
